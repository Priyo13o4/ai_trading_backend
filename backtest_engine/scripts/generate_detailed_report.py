...
async def analyze_set(session, start_date: datetime, end_date: datetime, label: str):
    query = text("""
    ...
    for idx, event in enumerate(valid_events):
        ...
        md += f"**Predicted Impact:** {str(event.market_impact_prediction).capitalize()}\n"
        md += f"**Score (Attention):** {event.attention_score} | **Confidence:** {event.analysis_confidence}\n\n"
        ...
        md += "| Pair | Dominant Impact | Pips Up | Pips Down | In Listed Instruments? |\n"
        md += "|---|---|---|---|---|\n"
        
        # We only want to list symbols that had significant moves or were explicitly requested
        primary = event.primary_instrument or ""
        forex_list = event.forex_instruments or []
        if isinstance(forex_list, str):
            import json
            try:
                forex_list = json.loads(forex_list)
            except:
                forex_list = []
                
        if not isinstance(forex_list, list):
            forex_list = []
            
        all_listed_symbols = [primary] + forex_list
        all_listed_symbols = [s.upper() for s in all_listed_symbols if s]
        ...
        for sym, sym_candles in symbol_data.items():
            ...
            if sym.upper() == primary.upper():
                presence = "Yes (Primary)"
            elif sym.upper() in all_listed_symbols:
                presence = "Yes (Forex List)"
                
            md += f"| {sym} | {direction.capitalize()} | {max_move_up:.1f} | {max_move_down:.1f} | {presence} |\n"
...
async def main():
    ...
    md_content = "# Full Asset Impact Breakdown\n\n"
    
    md_content += await analyze_set(session, set1_start, set1_end, "Set 1 (Jun 8 - Jun 11 18:00 UTC)")
    md_content += await analyze_set(session, set2_start, set2_end, "Set 2 (Jun 11 18:00 UTC - Latest)")
    
    await session.close()
    
    out_path = Path(__file__).parent.parent / "reports" / "news_impact" / "detailed_assets_breakdown.md"
