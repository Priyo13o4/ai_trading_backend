...
from .symbols import get_active_symbols, refresh_active_symbols
...
class MT5IngestServer:
    ...
    async def _get_bootstrap_symbols(self) -> list[str]:
        ...
        await refresh_active_symbols(redis_client=redis_client)
        syms = await get_active_symbols(redis_client=redis_client, fallback=["EURUSD"])
        if len(syms) > 255:
            syms = syms[:255]
        if self._debug:
...
def _write_candle_sync(
    ...
    from .timeframes import assert_timeframe_policy
